...
def make_square(img, size=220, color=(255,255,255)):
    w, h = img.size
    if w == h:
...
# 반응형 스타일: 최대폭 700px, 모바일/데스크탑 모두 자연스럽게
...
st.markdown(
    ...
    @media (max-width: 800px) {
        .main .block-container { max-width: 99vw; }
    }
    </style>
    """,
...
def get_image(file, fallback_path):
    if file is not None:
        ...
        img = Image.new("RGB", (220,220), (220,220,220))
...
# --- VS 이미지 생성 및 출력 ---
if submitted:
    # 반응형: 화면 폭에 따라 figure 크기 조정
    fig_width = 6.2 if st.session_state.get("is_mobile", False) else 7.2
